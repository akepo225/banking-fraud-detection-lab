"""Smoke and contract tests for the Alpine Crest production-monitoring notebook (v0.8)."""
...
def test_alpine_crest_monitoring_notebook_executes(tmp_path: Path) -> None:
    """The Alpine Crest monitoring notebook must execute on tiny data."""
    repo_root = Path(__file__).resolve().parents[1]
    output_name = "alpine_crest_monitoring.executed.ipynb"
...
def test_alpine_crest_monitoring_notebook_uses_v08_contracts() -> None:
...
def test_alpine_crest_monitoring_notebook_keeps_limitations_visible() -> None:
...
def test_alpine_crest_monitoring_notebook_uses_glossary() -> None:
