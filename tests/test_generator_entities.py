...
def test_generator_csv_writers_pin_utf8_encoding() -> None:
    """All generator CSV writers must explicitly pin encoding="utf-8".

    Regression guard for issue #162: ``minimal_world.py`` passed
    ``encoding="utf-8"`` while the digital/private ``_write_tables`` helpers
    omitted it. UTF-8 is the pandas default, so this guards explicitness and
    cross-file parity rather than behavior -- which is why it inspects source
    instead of generated bytes (a behavioral "valid UTF-8" check would pass even
    without the fix and so could not detect the inconsistency).
    """
    from banking_fraud_lab import generators

    minimal_src = inspect.getsource(generators.minimal_world)
    db_src = inspect.getsource(generators.digital_banking._write_tables)
    pb_src = inspect.getsource(generators.private_banking._write_tables)

    # Every to_csv(...) call site in minimal_world must carry the kwarg.
    assert minimal_src.count(".to_csv(") == minimal_src.count('encoding="utf-8"') >= 2
    # The digital/private helpers were the two that previously omitted it.
    assert 'encoding="utf-8"' in db_src
    assert 'encoding="utf-8"' in pb_src
