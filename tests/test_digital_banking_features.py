...
def test_digital_sql_beneficiary_passthrough_exercise_produces_valid_age_hours(
    tmp_path: Path,
) -> None:
    """SQL 10 must produce non-negative beneficiary age hours using datetime strings.

    This is a regression test for the fix that removed CAST(pb.created_at AS REAL)
    (which would silently return 0 or NULL) in favour of passing the ISO datetime
    ...
    try:
        ...
        assert "db_beneficiary_age_hours" in columns
        assert "db_is_new_beneficiary" in columns
        assert "db_is_rapid_pass_through" in columns

        # Rows with a known beneficiary must have a non-negative age in hours.
        rows_with_beneficiary = [
            row
            for row in rows
            if row["db_beneficiary_age_hours"] != -1
        ]
        assert rows_with_beneficiary, "Expected at least one row with a known beneficiary"
        for row in rows_with_beneficiary:
            assert row["db_beneficiary_age_hours"] >= 0, (
                f"Negative beneficiary age detected: {row['db_beneficiary_age_hours']}. "
...
def test_digital_sql_beneficiary_passthrough_exercise_uses_datetime_column_not_epoch() -> None:
    """SQL 10 must reference beneficiary_created_at (datetime), not beneficiary_created_epoch.

    The old column alias 'beneficiary_created_epoch' used CAST(pb.created_at AS REAL)
    which produced incorrect epoch arithmetic. The fix renames it to beneficiary_created_at
    and passes the ISO string directly to julianday().
    """
    ...
    assert "beneficiary_created_epoch" not in sql_text, (
        "SQL 10 still contains the old 'beneficiary_created_epoch' alias. "
...
def test_digital_sql_velocity_account_exercise_produces_valid_account_age_days(
    ...
    This is a regression test for the fix that removed CAST(a.opened_at AS REAL)
    (which would break julianday arithmetic) in favour of passing the ISO datetime
    string directly to julianday().
    """
    connection = create_minimal_banking_world_sqlite(
        tmp_path / "learner_world.sqlite", seed=42, scale="small"
...
def test_digital_sql_velocity_account_exercise_uses_datetime_column_not_epoch() -> None:
    """SQL 11 must reference opened_at (datetime column), not account_opened_epoch.

    The old alias 'account_opened_epoch' used CAST(a.opened_at AS REAL) which broke
    the julianday arithmetic in SQLite when opened_at is an ISO datetime string.
    """
    sql_text = DIGITAL_SQL_EXERCISES["db_velocity_account_features"].read_text(
        encoding="utf-8"
    )
    assert "account_opened_epoch" not in sql_text, (
        "SQL 11 still contains the old 'account_opened_epoch' alias. "
...
def test_digital_sql_velocity_account_exercise_early_life_boundary_is_inclusive(
    ...
    The old condition used <= 30 without a lower bound; the fix uses BETWEEN 0 AND 30
    which excludes negative values (impossible when dates are stored as ISO strings
    and accounts predate transactions, but guards against bad data).
    """
    sql_text = DIGITAL_SQL_EXERCISES["db_velocity_account_features"].read_text(
        encoding="utf-8"
    )
