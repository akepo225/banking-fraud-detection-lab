"""Generate the NovaBank interpretability + model-risk notebook (v0.7 #185).

Mirrors the generator-script convention used by the v0.6 graph notebooks and the
v0.7 Alpine Crest notebook: this script emits the deterministic ``.ipynb`` JSON
...
NOTEBOOK_PATH = NOTEBOOK_DIR / "novabank_interpretability.ipynb"
...
CELLS = [
    _md(
        ...
        "\n"
        "This notebook turns the v0.4 digital-banking model outputs into inspectable "
        "explanations and a model-risk review for **NovaBank Digital**. It is the second "
        "of two v0.7 interpretability track notebooks and consumes the explanation "
        "utilities (slice 1), the threshold / false-positive utilities (slice 2), and the "
        "model-documentation template (slice 3).\n"
        "\n"
        "Learning goal: explain *why* a specific digital alert was generated, separate "
        "rule, model, graph, and case signal types, choose a threshold that reflects alert "
        "capacity and cost, review where false positives concentrate, and keep the "
        "limitation-aware framing visible — **a model should not be judged by headline "
        "accuracy**.\n"
        "\n"
        "> Educational exercise on synthetic data. No real Client, account, or "
        "transaction data; no certification or legal-advice claim. The **User** is the "
        "digital login identity; the **Client** is the legal customer."
    ),
    _code(
        "import pandas as pd\n"
    ...
    _md(
        ...
        "\n"
        "The supervised label comes from generated case outcomes for the digital "
        "`new_beneficiary_payment`, `session_payment_velocity`, and `digital_scam_to_mule` "
        "Detection patterns."
    ),
    _code(
    ...
    _md(
        ...
        "\n"
        "Join cases, alerts, outcomes, and the v0.4 digital feature frame, then fit the "
        "same reproducible baseline used in the supervised baseline notebook so "
        "explanations are anchored to a real fitted model."
    ),
    _code(
    ...
    _md(
        ...
        "\n"
        "Feature importance explains which inputs drove each digital alert's score. The "
        "`ExplanationFamilySpec` vocabulary ties every feature to its **Detection pattern** "
        "(e.g. `digital_scam_to_mule`, `new_beneficiary_payment`, `session_payment_velocity`) "
        "so the explanation is traceable, not a black box. Importance is an inspection aid, "
        "not proof the model is correct — a model should not be judged by headline accuracy."
    ),
    _code(
    ...
    _md(
        ...
        "digital feature is swept across its observed range — a compact view of the model's "
        "marginal behaviour, tied to the same Detection pattern id."
    ),
    _code(
        "top_feature = str(\n"
        "    importance_overview.sort_values(\"native_importance\", ascending=False)\n"
    ...
    _md(
        ...
        "\n"
        "Digital-banking alerts draw on four signal types. Keeping them side by side lets a "
        "reviewer separate signal types: a model score, a deterministic rule trigger, the "
        "v0.6 **graph** signal (`mule_ring` / `digital_scam_to_mule` network), and the v0.5 "
        "**case** narrative. Each adds something the others do not.\n"
        "\n"
    ...
    _md(
        ...
        "investigators and inflates false-positive cost. The recommender sweeps alert "
        "capacity and the investigation / false-positive / missed-fraud costs so the chosen "
        "threshold reflects those tradeoffs rather than a default."
    ),
    _code(
    ...
    _md(
        ...
        "\n"
        "Where do false positives fall? Grouping by `alert_type` shows whether review burden "
        "concentrates on one digital pattern — a review prompt, not a fairness verdict."
    ),
    _code(
    ...
    _md(
        "## Model Documentation\n"
        "\n"
        "The frozen template fills every required section (purpose, data lineage, "
        "assumptions, limitations, monitoring needs) deterministically from the fitted model, "
        "producing a stakeholder-readable artifact for the governance memo."
    ),
    _code(
