"""Generate the v0.7 governance memo notebook (#186).

Mirrors the generator-script convention: emits the deterministic ``.ipynb`` JSON
so the notebook is reproducible. Regeneration is optional and manual.
...
NOTEBOOK_PATH = NOTEBOOK_DIR / "governance_memo.ipynb"
...
CELLS = [
    _md(
        ...
        "\n"
        "This notebook converts the technical outputs from both v0.7 interpretability track "
        "notebooks (Alpine Crest Private Bank and NovaBank Digital) into a single "
        "stakeholder-readable governance summary. It consumes the model-documentation template "
        "and monitoring checklist (slice 3) and synthesises explanations, threshold decisions, "
        "false-positive concentration, model documentation, and monitoring needs for both tracks.\n"
        "\n"
        "Governance language stays educational: this memo is an exercise, not a certification or "
        "legal-advice artifact. **A model should not be judged by headline accuracy.**\n"
        "\n"
    ...
    _md(
        ...
        "\n"
        "Reproduce the two fitted baselines (Alpine Crest private-banking and NovaBank Digital) "
        "so the memo synthesises real model outputs rather than hand-written numbers. Each track "
        "produces a fitted model, an alert-score frame, a threshold recommendation, and "
    ...
    _md(
        ...
        "\n"
        "For each track, run the lowest-cost-threshold recommender across capacity and cost, then "
        "collect the recommended threshold, total cost, recall, and capacity note into one summary "
        "table the memo can cite."
    ),
    _code(
        "threshold_rows = []\n"
        "for track in tracks:\n"
    ...
    _md(
        ...
        "\n"
        "For each track, summarise the top feature-importance driver (the explanation the "
        "stakeholder memo cites) and the false-positive concentration by alert_type at the "
        "recommended threshold. These feed the memo's explanation and review-burden sections."
    ),
    _code(
        "explanations = {}\n"
        "fp_concentrations = {}\n"
    ...
    _md(
        ...
        "\n"
        "Use the model-documentation template and monitoring checklist (slice 3) to produce, for "
        "each track, a documentation artifact and a monitoring checklist. These feed the memo's "
        "documentation and monitoring sections."
    ),
    _code(
    ...
    _md(
        "## Governance Memo\n"
        "\n"
        "The memo below is generated from the synthesised outputs. It is structured for a "
        "stakeholder audience: threshold decisions, model documentation status, monitoring needs, "
        "and limitations. Governance language is educational — no certification or legal-advice "
        "claims."
    ),
    _code(
        ...
        "    parts = [f\"{len(ok)} of {len(statuses)} dimensions ok\"]\n"
        "    if review:\n"
    ...
    _md(
        ...
        "\n"
        "Confirm the memo carries no certification or legal-advice claims and keeps the "
        "limitation-aware framing visible. The repository stays private; this memo adds no "
