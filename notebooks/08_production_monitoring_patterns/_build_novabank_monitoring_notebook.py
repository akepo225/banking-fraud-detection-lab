"""Generate the NovaBank Digital production-monitoring notebook (v0.8 #208).

Mirrors the generator-script convention used by the Alpine Crest monitoring
notebook (#207) and the earlier v0.6 graph / v0.7 interpretability notebooks:
this script emits the deterministic ``.ipynb`` JSON so the notebook content is
reproducible and reviewable as code. Regeneration is optional and manual — the
committed ``.ipynb`` is authoritative.

Run from the repo root::

    uv run python notebooks/08_production_monitoring_patterns/_build_novabank_monitoring_notebook.py
"""
...
NOTEBOOK_PATH = NOTEBOOK_DIR / "novabank_monitoring.ipynb"
...
CELLS = [
    _md(
        ...
        "\n"
        "This notebook runs the v0.8 production-monitoring flow for **NovaBank Digital** "
        "(digital-banking track). It is the digital-banking counterpart of the Alpine Crest "
        "monitoring notebook and consumes the same v0.8 monitoring builders — batch scoring, "
        "alert decisions, reviewer/audit, alert-queue inspection, operational metrics, "
        "score/feature drift, and monitoring data-quality — on top of the v0.4 digital-banking "
        "supervised baseline and the v0.7 threshold recommender / feature explanations.\n"
        "\n"
        "Learning goal: trace one digital Detection pattern (`digital_scam_to_mule`) from a "
        "fitted score, through threshold selection, into the production score / threshold / "
        ...
        "data; no production-readiness, certification, or legal-advice claim. The **User** is "
        "the digital login identity; the **Client** is the legal customer."
    ),
    _code(
        "import pandas as pd\n"
    ...
    _md(
        ...
        "\n"
        "The supervised label comes from generated case outcomes for the digital-banking "
        ...
        "are produced exactly as in the v0.7 NovaBank interpretability notebook, so the "
        "monitoring flow below starts from the same reproducible score. The frame also "
        "carries Banking relationship and User lineage so each monitoring score row traces "
        "back to a Client-facing record."
    ),
    _code(
    ...
    _md(
        ...
        "and `threshold` tables for the `digital_scam_to_mule` Detection pattern."
    ),
    _code(
    ...
    _md(
        ...
        "Each decision carries forward the score, threshold, Banking relationship, and User "
        "lineage."
    ),
    _code(
    ...
    _md(
        ...
        "top feature driver for `digital_scam_to_mule` from the fitted baseline."
    ),
    _code(
