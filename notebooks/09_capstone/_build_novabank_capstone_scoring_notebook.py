"""Generate the NovaBank capstone scoring notebook (v0.9 #227).
...
The capstone notebook reuses — does NOT reimplement — the v0.4 digital-banking
...
NOTEBOOK_PATH = NOTEBOOK_DIR / "novabank_capstone_scoring.ipynb"
...
CELLS = [
    _md(
        ...
        "\n"
        "This is the v0.9 capstone notebook for the **Digital-banking fraud detection** track "
        "at **NovaBank Digital**. It runs the full capstone loop from the scenario brief: "
        "generate the capstone dataset, build the engineered feature frame, fit a transparent "
        "scoring rule, review alerts with alert-aware metrics, choose a capacity-aware "
        "threshold, and explain *why* each alert fired — without leaving the capstone flow.\n"
        "\n"
        "It reuses the v0.4 digital-banking feature library, the v0.1 alert-aware evaluation, "
        "the capacity-aware threshold recommender, and the v0.7 per-alert explanations. The "
        "investigation targets the `digital_scam_to_mule` and `new_beneficiary_payment` "
        "**Detection patterns** over the capstone substrate. Graph evidence (`mule_ring`) is "
        "investigative support and is covered in the synthesis notebook. Recall the glossary: "
        "the **User** is the digital login identity, distinct from the **Client** who holds the "
    ...
    _md(
        ...
        "\n"
        "Join cases, alerts, outcomes, and the v0.4 digital-banking feature frame into one "
    ...
    _md(
        "## Capstone Takeaways\n"
        "\n"
        "This notebook ran the full digital-banking capstone loop on the v0.9 capstone dataset: "
        ...
        "synthesis notebook adds graph (`mule_ring`) and case evidence as investigative support "
        "and renders the governance memo for both tracks.\n"
        "\n"
